...
def is_important(title):
    popular_df = get_wiki_popular_pages()
    return (
        popular_df[popular_df["Page title"] == title]["Importance"].values[0]
        if is_popular(title)
        else "na"
    )
...
def get_wiki_pages_overview(category="Climate_change"):
    ...
    for title_code in tqdm(wiki_titles[category]):
        ...
        if len(init_content_df) == 0:
            ...
            continue
        last_content = _df[_df["timestamp"] == _df["timestamp"].max()][
            "content"
        ].values[0]
        edits_w_latest_content_df = _df[(_df["content"] == last_content)]
        last_updated_df = edits_w_latest_content_df[
            edits_w_latest_content_df["timestamp"]
            == edits_w_latest_content_df["timestamp"].min()
        ]
...
def get_revision_count(
    ...
    for title_code in tqdm(wiki_titles[category]):
        ...
        edits_count_df = (
            _df["abstracted_date"].value_counts().to_frame(name="edits_count")
        )
...
def batch_revisions_per_period(
...
def get_wikipedia_page_periodic_overview(
    ...
    for title_code in tqdm(wiki_titles[category]):
        ...
        for period_val, period_df in _df.groupby("abstracted_date"):
            latest_df = period_df[
                period_df["timestamp"] == period_df["timestamp"].max()
            ]
            revision_per_period = {}
