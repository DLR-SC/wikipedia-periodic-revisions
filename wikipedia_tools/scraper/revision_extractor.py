...
def process_page_revisions(
    page_name, lang, rev_from: datetime = None, rev_to: datetime = None
):
    res = None
    error = False
    disambiguity = {}
    ...
    try:
        ...
        try:
            ...
            print(
                f"..found disambiguity for page *{page_name}*, fetching  revisions for options: {e.options}"
            )
            disambiguity[page_name] = (
                e.options
                if page_name not in disambiguity.keys()
                else disambiguity[page_name].extend(e.options)
            )
            dict_ = {}
            for p in e.options:
        ...
        if data is not None:
            res = [
                {
                    **{"page": page_name, "lang": lang, "timestamp": item[0]},
                    **dict(item[1]),
                }
