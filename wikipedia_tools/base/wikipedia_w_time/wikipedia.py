...
class WikipediaPage():
  ...
  def __load(self, redirect=True, preload=False):
    ...
    # then the page must be a disambiguation page
      ...
      if self.rev_from is not None:
        query_params['rvstart'] = self.rev_from.strftime(WikipediaPage.DATE_FORMAT)
        query_params['rvend'] = datetime.now().strftime(WikipediaPage.DATE_FORMAT)  if self.rev_to is None else self.rev_to.strftime(WikipediaPage.DATE_FORMAT)
        query_params['rvdir'] = 'newer'

        if query_params['rvstart'] > query_params['rvend']:
  ...
  def html(self):
    ...
    if not getattr(self, '_html', False):
      ...
      if self.rev_from is not None:
        query_params['rvstart'] = self.rev_from.strftime(WikipediaPage.DATE_FORMAT)
        query_params['rvend'] = datetime.now().strftime(WikipediaPage.DATE_FORMAT) \
          if self.rev_to is None \
          else self.rev_to.strftime(WikipediaPage.DATE_FORMAT)
  ...
  def revisions(self):
    ...
    today = datetime.now().strftime(WikipediaPage.DATE_FORMAT)
    ...
    if not getattr(self, '_revision', False):
      ...
      if self.rev_from is not None:
        query_params['rvstart'] = self.rev_from.strftime(WikipediaPage.DATE_FORMAT)
        query_params['rvend'] = datetime.now().strftime(
          WikipediaPage.DATE_FORMAT) \
          if self.rev_to is None \
          else self.rev_to.strftime(WikipediaPage.DATE_FORMAT)
        query_params['rvdir'] = 'newer'
