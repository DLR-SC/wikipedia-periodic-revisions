...
def extract_nrc_emotion(df, content_cols, file_path=None):
    ...
    for col in content_cols:
        ...
        nrc_col_df = lexicons.count_nrc_emotions_and_sentiments(
            data_df, text_column=col, prefix="nrc_" + col + "_"
        )
...
def extract_empath(wiki_page_title, content_col="content", file_path=None):
    ...
    empath_df = lexicons.count_empath(
        empath_df, text_column=content_col, prefix="empath_"
    )
...
def extract_mpqa_arg(df, content_cols, file_path=None):
    ...
    for col in content_cols:
        ...
        nrc_col_df = lexicons.count_mpqa_arg(
            data_df, text_column=col, prefix="mpqa_arg_" + col + "_"
        )
